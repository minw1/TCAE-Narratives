...
class POS_Predictor_Trainer:
    ...
    def __init__(self, options, data_module):
        ...
        if not self.options.decoder_base == "none":
    ...
    def log_epoch(self, epoch):
        ...
        train_acc = eval_tres["acc"]

        self.writer.add_scalar('Train_Loss', train_loss, epoch)
        self.writer.add_scalar('Val_Loss', val_loss, epoch)

        self.writer.add_scalar('Train_Perp', train_perp, epoch)
        self.writer.add_scalar('Val_Perp', val_perp, epoch)

        self.writer.add_scalar('Train_Acc', train_acc, epoch)
        self.writer.add_scalar('Val_Acc', val_acc, epoch)

        return(eval_res, eval_tres)

    def train(self):
        #once before any training
        self.log_epoch(self.start_epoch)
        ...
        for epoch in range(self.start_epoch, self.end_epoch):
            ...
            self.writer.add_scalar('Train_Perp', train_perp, epoch+1)
            self.writer.add_scalar('Val_Perp', val_perp, epoch+1)

            self.writer.add_scalar('Train_Acc', train_acc, epoch+1)
            self.writer.add_scalar('Val_Acc', val_acc, epoch+1)
    ...
    def train_epoch(self, epoch):
        self.model.train()
        if self.options.enc_freeze_type == "all":
            self.model.module.encoder.eval()
            for param in self.model.module.encoder.parameters():
                param.requires_grad = False
        if self.options.dec_freeze_type == "thaw_cross":
            for param in self.model.module.decoder.parameters():
                param.requires_grad = False
            for name, param in self.model.module.decoder.named_parameters():
    ...
    def load(self):
        ...
        self.model = build_pos_predictor(self.options).to(device)

        checkpoint_tmp = torch.load(self.options.checkpoint, map_location=device)

        print(f"Load checkpoint {checkpoint_tmp['epoch']} with loss {checkpoint_tmp['best_val_loss']}")
        ...
        self.optimizer = build_optimizer(
            self.options.lr, self.options.weight_decay, self.model.parameters()
        )
        self.optimizer.load_state_dict(checkpoint_tmp['optimizer'])

        self.scheduler = build_lr_scheduler(
            self.options, self.optimizer, self.start_epoch
        )
    ...
    def train_epoch(self, epoch):
        ...
        for iter, data in enumerate(self.train_loader):
            ...
            if iter % self.options.report_period == 0:
                ...
                self.writer.add_scalar('Train_Acc', train_acc, iter)
                self.writer.add_scalar('Val_Acc', val_acc, iter)
