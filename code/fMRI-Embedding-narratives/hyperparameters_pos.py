        ...
        for wd in [1e-6]:
            for dec_ff in [512]:
                for encoder in [large_encoder]:
                    ...
                    print(f'python rt_main.py --num_epochs 100 --predict --exp_dir /home/wsm32/project/wsm_thesis_scratch/narratives/rt_large/pos_predict/{dec_blocks}-{dec_dropout}-{wd}-{dec_ff}-{encoder["kind"]} --encoder_base /home/wsm32/project/wsm_thesis_scratch/narratives/rt_large/{encoder["lr"]}-{encoder["lr_decay"]}-{encoder["layer_count"]}-{encoder["latent"]}-{encoder["dropout"]}-{encoder["head_count"]}-{encoder["batch_size"]}/best_model.pt --data_parallel --report_period 10 --early_stop 100 --lr {lr} --lr_gamma {lr_decay} --lr_step_size 15 --n_head {encoder["head_count"]} --layers {encoder["layer_count"]} --dropout {encoder["dropout"]} --d_latent {encoder["latent"]} --n_dec_blocks {dec_blocks} --dec_dropout {dec_dropout} --weight_decay {wd} --d_dec_ff {dec_ff} --batch_size {batch_size} --l_vocab 20\n')
