...
class RT_Narrative_Data_Module():  # for random time splits
    ...
    def __init__(
        ...
        num_workers: int = 2,
        train_val_test: tuple = (0.8, 0.1, 0.1),  # proportion of dataset assigned to train, validation, test
        segment_length: int = 10,  # trs per segment
        delay: float = 4.5,
        tr_duration = 1.5,
        random_seed = 0,
        max_length = 100,
        scramble_labels: bool = False,      # <-- new parameter for permutation test
        scramble_seed: Optional[int] = None   # <-- optional seed for scrambling for reproducibility
        ):
        ...
        for task in self.task_list:
            ...
            train_loaders.append(
                RT_Narrative_Dataset(
                    ...
                    scramble_labels=self.scramble_labels,
...
class RT_Narrative_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        max_length,
        transform: Optional[Callable] = None,
        scramble_labels: bool = False,      # <-- new parameter
        scramble_seed: Optional[int] = None   # <-- optional seed for reproducibility
    ):
        """
        Args:
            task: The current task.
            task_data: Dictionary containing the task-specific data.
            segment_length: Number of TRs per segment.
            train_val_test: Tuple specifying the proportions for train, val, test.
            delay: Delay parameter.
            tr_duration: Duration of each TR.
            which_type: 0 for train, 1 for validation, 2 for test.
            max_length: Maximum length for the label sequence.
            transform: Optional transformation to apply to the signal.
            scramble_labels: If True, scramble the labels for permutation testing.
            scramble_seed: Seed for scrambling (for reproducibility).
        """
        self.task = task
        ...
        self.segs_per_sub = np.sum(task_data["pattern"] == which_type)  # segments per subject (in this partition)
        self.num_ex = self.segs_per_sub * task_data["num_subj"]          # total examples for this dataset
        ...
        if self.scramble_labels:
            # Create a fixed random permutation for the labels
            rng = np.random.default_rng(scramble_seed) if scramble_seed is not None else np.random.default_rng()
            self.label_perm = rng.permutation(self.num_ex)
        
        print("Task Loader, #" + str(which_type) + " has " + str(task_data["num_subj"]) +
              " subjects and " + str(self.segs_per_sub) + " segments per subject.")

    def seg_idx_to_trs(self, seg_idx):  
        # Converts a segment index to the relevant TR indices
    ...
    def __getitem__(self, i: int):
        # Get brain sample (signal) for index i
        ...
        # If scrambling is enabled, use a label from a random (permuted) index.
        label_idx = i
        if self.scramble_labels:
            label_idx = self.label_perm[i]

        # Convert the label index into the corresponding subject and segment
        subj_label = math.floor(label_idx / self.segs_per_sub)
        seg_label = label_idx % self.segs_per_sub
        tr_span_label = self.seg_idx_to_trs(self.these_seg_ids[seg_label])
        pos_seq = get_pos_seq(
            self.task_data["align"],
            tr_span_label[0],
            tr_span_label[1],
            self.task_data["stim_offset"],
            tr_dur=self.tr_duration,
            delay=self.delay
        )

        # Pad label to max_length
        if len(pos_seq) >= self.max_length:
            raise ValueError("pos_seq length exceeds max_length.")
        label = np.full(self.max_length, pos_tags["PAD"])
        label[:len(pos_seq)] = pos_seq

        #if self.scramble_labels:
        #    return torch.from_numpy(np.zeros_like(sample)), label
